...
* Week 7
1. Download [[https://web.tresorit.com/l/LgFab#pgfSdjSm0wf4oAtRpkybYQ][this dataset]] on German hospitals. Open the dataset in Julia as a DataFrame using the CSV.jl and DataFrames.jl package. The meaning of some of the variables is given in the table below. Use the "describe" command to get some descriptive statistics. How many births are there on average in a hospital? How many Caesarean sections? Compute these variables only for those hospitals for which the number of births is not missing and strictly greater zero. 
